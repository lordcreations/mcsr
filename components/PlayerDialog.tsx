  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg w-full dark:bg-gray-900 bg-white text-black dark:text-white font-minecraft border border-gray-300 dark:border-gray-700 shadow-lg bg-[url('/textures/stone.png')] bg-repeat">
        ...
        {loading && (
          <p className="text-center mt-4 font-minecraft">Carregando...</p>
        )}
                  ...
                  {[
                    {
                      icon: "📅",
                      label: "Entrou",
                      value: validDate(data?.data?.timestamp?.firstOnline),
                    },
                    {
                      icon: "🗓️",
                      label: "Ultimo Login",
                    ...
                    },
                    {
                      icon: "🎯",
                      label: "Pontos",
                      value: data?.data?.eloRate ?? "N/A",
                    },
                    {
                      icon: "🏅",
                      label: "Posicao",
                      value: data?.data?.eloRank ?? "N/A",
                    },
                    {
                      icon: "📈",
                      label: "Maior Elo",
                      value: data?.data?.seasonResult?.highest ?? "N/A",
                    },
                    {
                      icon: "📉",
                      label: "Menor Elo",
                      value: data?.data?.seasonResult?.lowest ?? "N/A",
                    },
                  ].map((item, i) => (
                    <div key={i} className="flex items-start justify-between">
                      ...
                      <span className="text-sm font-chat mt-2">
                        {item.value}
                      </span>
                    </div>
                  ))}
                  ...
                  <>
                    <div className="space-y-1 px-3 py-2 rounded-md bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 shadow-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <span>
                          {data.data.seasonResult.last?.eloRate ?? "N/A"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>🏅 Ultimo Rank</span>
                        <span>
                          {data.data.seasonResult.last?.eloRank ?? "N/A"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>💠 Pontos da Fase</span>
                        <span>
                          {data.data.seasonResult.last?.phasePoint ?? 0}
                        </span>
                      </div>
                    </div>
                    ...
                    {data.data.seasonResult.phases?.length ? (
                      <div className="space-y-2">
                        ...
                        <ul className="list-disc list-inside pl-2 space-y-1">
                          {data.data.seasonResult.phases.map(
                            (phase: any, index: number) => (
                              ...
                              >
                                Fase {phase.phase}: Elo {phase.eloRate}, Rank{" "}
                                {phase.eloRank}, Pontos {phase.point}
                              </li>
                            )
                          )}
                        </ul>
                      ...
                      <p className="text-sm italic text-gray-500">
                        Nenhum historico detalhado de fases disponivel.
                      </p>
                    )}
                  </>
                ) : (
              ...
              <div className="text-base text-gray-700 dark:text-gray-300 space-y-1">
                {data.data.achievements?.display?.length ? (
                  <ul className="list-disc list-inside">
                    {data.data.achievements.display.map(
                      (ach: any, index: number) => (
                        <li key={index}>
                          {ach.id.replace(/([A-Z])/g, " $1").trim()}
                        </li>
                      )
                    )}
                  </ul>
